/*
 * Class KI_CHARACTER_BUFFER
 */

#include "eif_macros.h"


#ifdef __cplusplus
extern "C" {
#endif

static const EIF_TYPE_INDEX egt_0_1189 [] = {0xFF01,232,0xFFFF};
static const EIF_TYPE_INDEX egt_1_1189 [] = {0xFF01,245,1188,0xFFFF};
static const EIF_TYPE_INDEX egt_2_1189 [] = {0xFF01,1188,0xFFFF};
static const EIF_TYPE_INDEX egt_3_1189 [] = {0,0xFFFF};
static const EIF_TYPE_INDEX egt_4_1189 [] = {0,0xFFFF};
static const EIF_TYPE_INDEX egt_5_1189 [] = {0xFF01,1188,0xFFFF};
static const EIF_TYPE_INDEX egt_6_1189 [] = {0xFF01,1188,0xFFFF};
static const EIF_TYPE_INDEX egt_7_1189 [] = {0,0xFFFF};
static const EIF_TYPE_INDEX egt_8_1189 [] = {0xFF01,15,0xFFFF};
static const EIF_TYPE_INDEX egt_9_1189 [] = {0xFF01,232,0xFFFF};
static const EIF_TYPE_INDEX egt_10_1189 [] = {0xFF01,232,0xFFFF};
static const EIF_TYPE_INDEX egt_11_1189 [] = {0xFF01,25,0xFFFF};
static const EIF_TYPE_INDEX egt_12_1189 [] = {0xFF01,1188,0xFFFF};
static const EIF_TYPE_INDEX egt_13_1189 [] = {194,0xFFFF};
static const EIF_TYPE_INDEX egt_14_1189 [] = {0xFF01,1187,0xFFFF};
static const EIF_TYPE_INDEX egt_15_1189 [] = {0xFF01,965,0xFFFF};
static const EIF_TYPE_INDEX egt_16_1189 [] = {0xFF01,232,0xFFFF};
static const EIF_TYPE_INDEX egt_17_1189 [] = {0xFF01,232,0xFFFF};
static const EIF_TYPE_INDEX egt_18_1189 [] = {400,194,0xFFFF};
static const EIF_TYPE_INDEX egt_19_1189 [] = {0xFF01,232,0xFFFF};


static const struct desc_info desc_1189[] = {
	{EIF_GENERIC(NULL), 0xFFFFFFFF, 0xFFFFFFFF},
	{EIF_GENERIC(egt_0_1189), 0, 0xFFFFFFFF},
	{EIF_GENERIC(egt_1_1189), 1, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 2, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 3, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 4, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 5, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 6, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 7, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 8, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0191 /*200*/), 9, 0xFFFFFFFF},
	{EIF_GENERIC(egt_2_1189), 10, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 11, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 12, 0xFFFFFFFF},
	{EIF_GENERIC(egt_3_1189), 13, 0xFFFFFFFF},
	{EIF_GENERIC(egt_4_1189), 14, 0xFFFFFFFF},
	{EIF_GENERIC(egt_5_1189), 15, 0xFFFFFFFF},
	{EIF_GENERIC(egt_6_1189), 16, 0xFFFFFFFF},
	{EIF_GENERIC(egt_7_1189), 17, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 18, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 19, 0xFFFFFFFF},
	{EIF_GENERIC(egt_8_1189), 20, 0xFFFFFFFF},
	{EIF_GENERIC(egt_9_1189), 21, 0xFFFFFFFF},
	{EIF_GENERIC(egt_10_1189), 22, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 23, 0xFFFFFFFF},
	{EIF_GENERIC(egt_11_1189), 24, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 25, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 26, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 27, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0949 /*1188*/), 28, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x01C7 /*227*/), 29, 0xFFFFFFFF},
	{EIF_GENERIC(egt_12_1189), 30, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x0185 /*194*/), 0x00, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x01BB /*221*/), 0x00, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 0x00, 0xFFFFFFFF},
	{EIF_NON_GENERIC(0x01BB /*221*/), 17388, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 15201, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 15202, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 0x00, 0xFFFFFFFF},
	{EIF_GENERIC(egt_13_1189), 0x00, 0xFFFFFFFF},
	{EIF_GENERIC(egt_14_1189), 16922, 0xFFFFFFFF},
	{EIF_GENERIC(egt_15_1189), 17191, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 0x00, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 17383, 0xFFFFFFFF},
	{EIF_GENERIC(egt_16_1189), 0x00, 0xFFFFFFFF},
	{EIF_GENERIC(egt_17_1189), 17384, 0xFFFFFFFF},
	{EIF_GENERIC(egt_18_1189), 17385, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 17386, 0xFFFFFFFF},
	{EIF_GENERIC(NULL), 17387, 0xFFFFFFFF},
	{EIF_GENERIC(egt_19_1189), 17389, 0xFFFFFFFF},
};
void Init1189(void)
{
	IDSC(desc_1189, 0, 1188);
	IDSC(desc_1189 + 1, 1, 1188);
	IDSC(desc_1189 + 32, 556, 1188);
	IDSC(desc_1189 + 40, 560, 1188);
	IDSC(desc_1189 + 41, 559, 1188);
	IDSC(desc_1189 + 42, 478, 1188);
}


#ifdef __cplusplus
}
#endif
